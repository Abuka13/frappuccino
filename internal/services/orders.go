...
// 					similarity(name, $1) + similarity(description, $1) AS relevance
// 				FROM menu_items
...
// 			}
// 			menuQuery += " ORDER BY relevance DESC LIMIT 10"
...
// 			}
// 			orderQuery += " GROUP BY o.id, c.name, o.total_amount ORDER BY relevance DESC LIMIT 10"
...
// 			daysInMonth := time.Date(year, time.Month(monthNum+1), 0, 0, 0, 0, time.UTC).Day()
